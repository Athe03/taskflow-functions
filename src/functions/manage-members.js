...
app.http('manage-members', {
    ...
    handler: async (request) => {
        ...
        // Verifier que l'appelant est admin ou owner.
        ...
        if (!callerRole || !['admin', 'owner'].includes(callerRole.role)) {
            return { status: 403, jsonBody: { error: 'Admin requis' } };
        }
        ...
        if (action === 'remove') {
            ...
            if (target?.role === 'owner') {
                return { status: 403, jsonBody: { error: 'Impossible de retirer le owner' } };
            }
        ...
        return { status: 400, jsonBody: { error: 'action invalide' } };
    }
});